# File: image_searcher.py
# A unified image searcher with switchable backends: true VL search vs. OCR-based search.
...
class ImageSearcher(BaseSearcher):
    """
    一个统一的图像检索器，支持两种模式:
    1. 'vl_search' (默认): 真正的视觉-语言（VL）多模态检索，理解图像内容。
    2. 'ocr_search': 高效的、基于OCR文本的代理检索，擅长查找含特定文字的图像。
    """
    ...
    def __init__(self,
        ...
        if self.mode == 'vl_search':
            ...
            self._init_ocr_mode(vl_node_dir_prefix, ocr_node_dir_prefix, ocr_text_encoder, img_dir_prefix)
        else:
    ...
    def _init_vl_mode(self, node_dir_prefix, model_name):
        if node_dir_prefix is None:
            if 'colqwen' in model_name: node_dir_prefix = 'colqwen_ingestion'
        ...
        self.embed_model = VL_Embedding(model=model_name, mode='image')
        self.metadata_store = []
    ...
    def _load_vl_index(self):
        print("Applying memory-optimized streaming load for VL Image Engine...")
        embedding_tensors = []
        ...
        print(f"VL index loaded. Found {len(self.metadata_store)} documents.")
    ...
    def _init_ocr_mode(self, vl_node_dir, ocr_node_dir, text_encoder, img_dir):
        if vl_node_dir is None or ocr_node_dir is None:
            raise ValueError("vl_node_dir_prefix and ocr_node_dir_prefix must be provided for OCR mode.")
        
        self.vl_node_dir = os.path.join(self.dataset_dir, vl_node_dir)
        self.ocr_dir = os.path.join(self.dataset_dir, ocr_node_dir)
        self.img_dir = os.path.join(self.dataset_dir, img_dir)
        self.text_encoder = text_encoder
        self.cache_dir = ".cache/ocr_image_embeds"
        os.makedirs(self.cache_dir, exist_ok=True)
        
        self.stems = sorted([os.path.splitext(os.path.basename(p))[0] for p in glob.glob(os.path.join(self.vl_node_dir, "*.node"))])
        print(f"OCR mode: Found {len(self.stems)} image pages.")
    ...
    def _load_or_build_ocr_cache(self) -> np.ndarray:
        # This is a simplified cache logic. A robust version would use hashing like in your example.
        cache_file = os.path.join(self.cache_dir, f"{os.path.basename(self.dataset_dir)}_{self.text_encoder.replace('/','_')}.npy")
        if os.path.exists(cache_file):
            print(f"OCR mode: Loading embeddings from cache: {cache_file}")
            return np.load(cache_file)
        
        print("OCR mode: Building OCR text embeddings...")
        ...
        print("OCR mode: Embeddings cached.")
        return emb_np
    ...
    def _search_ocr(self, query: str, top_k: int) -> List[NodeWithScore]:
        ...
        for i in sorted_idx:
            stem = self.stems[i]
            img_path = os.path.join(self.img_dir, f"{stem}.jpg") # Assuming jpg
            node = ImageNode(
                id_=stem,
                text=self.texts[i] or f"",
